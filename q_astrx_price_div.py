# ASTRX Price and Dividend Analysis - Quarterly Data
...
def load_and_clean_data():
    """
    Load and clean the ASTRX index and dividend yield data from Excel file
    """
    # File path
    file_path = r"C:\Users\Manpreet\OneDrive - City St George's, University of London\Documents\Term1\Coursework\SMM265 Asset Pricing\Q1\Data\Raw Data\Quarterly\SMM265 - ASTRX - FTSE All Share Index - Quarterly with Dividend Yield.xlsx"
    ...
    try:
        ...
        # Load the Excel file
        print("Loading ASTRX Excel file...")
        ...
        # Rename columns to 'date', 'astrx_index', 'astrx_div_yield'
        if len(df_cleaned.columns) >= 3:
            df_cleaned.columns = ['date', 'astrx_index', 'astrx_div_yield']
            print(f"Columns renamed to: {list(df_cleaned.columns)}")
        else:
            print(f"Warning: Expected at least 3 columns, found {len(df_cleaned.columns)}")
        ...
        print(f"Error loading/cleaning ASTRX data: {e}")
        return None

def save_cleaned_data(df, output_path=None):
    """
    Save cleaned ASTRX data to Excel file
    """
    if df is None:
    ...
    if output_path is None:
        ...
        output_path = os.path.join(output_dir, "SMM265 - ASTRX - FTSE All Share Index - Quarterly with Dividend Yield.xlsx")
    
    try:
        df.to_excel(output_path, index=False)
        print(f"\nCleaned ASTRX data saved to: {output_path}")
    except Exception as e:
        print(f"Error saving ASTRX data: {e}")

def main():
    """
    Main function for ASTRX index and dividend analysis
    """
    print("ASTRX Index and Dividend Analysis - Quarterly")
    print("=" * 40)
